...
import (
	...
	bclient "go.unistack.org/micro-broker-grpc/v3"
...
func main() {
	...
	_, err = b.Subscribe(ctx, "topic-2", h.Handle)
	if err != nil {
		log.Fatal(ctx, "client: subscribe error", err)
	}
	_, err = b.Subscribe(ctx, "topic-3", h.Handle)
	if err != nil {
		log.Fatal(ctx, "client: subscribe error", err)
	}
	_, err = b.Subscribe(ctx, "topic-4", h.Handle)
	if err != nil {
		log.Fatal(ctx, "client: subscribe error", err)
	}
	_, err = b.Subscribe(ctx, "topic-5", h.Handle)
	if err != nil {
		log.Fatal(ctx, "client: subscribe error", err)
	}
	svc.Init()
